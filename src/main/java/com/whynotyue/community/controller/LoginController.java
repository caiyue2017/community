...
public class LoginController implements CommunityConstant {
    ...
    // 只要表单提交的参数跟User类中属性匹配，SpringMVC会自动完成注入
    ...
    // 不写在getLoginPage()中，因为这是一个异步的过程，点击刷新验证码，而不用刷新整个登录页面
    ...
    public void getKaptcha(HttpServletResponse response/*, HttpSession session*/) {
        ...
        // 生成验证码归属，存放到Cookie，发送给客户端
        String kaptchaOwner = CommunityUtil.generateUUID();
        Cookie cookie = new Cookie("kaptchaOwner", kaptchaOwner);
        cookie.setMaxAge(60);
        cookie.setPath(contextPath);
        response.addCookie(cookie);
        // 将验证码存入 Redis，用的是String类型，60S后过期
    ...
    // SpringMVC将实体类自动注入Model中，但基本数据类型、String等不会，手动注入或者页面从request中取
    public String login(String username, String password,
                        Model model, HttpServletResponse response) {


        // 从Redis中检查验证码
        // String code, @CookieValue("kaptchaOwner") String kaptchaOwner
        /*String kaptcha = null;
        ...
        if (map.containsKey("ticket")) { // 登录凭证放入 cookie 中
